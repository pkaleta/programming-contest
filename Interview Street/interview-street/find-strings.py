...
def updateCounts(node):
    ...
    for n in node.children.values():
        ret += 1 + updateCounts(n)
    node.count = ret
    return ret

def find(k, cur, ret):
    total = 0
    if cur is None: return "INVALID"
    if k == 0: return ret

    for x in xrange(ord('a'), ord('z') + 1):
        c = chr(x)
        if cur.children.has_key(c):
            count = cur.children[c].count
            if total + count + 1 >= k:
                return find(k - total - 1, cur.children[c], ret + c)
            else:
                total += count + 1
    return "INVALID"
