# I have used publicly available code for clustering using cosine distance
...
def get_documents(s):
    texts = []
    for doc in json.loads(s):
        texts.append(str(doc['id']) + '\n' + doc['title'] + '\n' + doc['content'])
    return [{"text": text, "tokens": text.split()}
             for i, text in enumerate(texts)]

def main(args):
    documents = get_documents(sys.stdin.read())
    ...
    for cluster in majorclust(dist_graph):
        ...
        for doc_id in cluster:
            cl.append(int(documents[doc_id]["text"].split()[0]))
        clusters.append(sorted(cl))
